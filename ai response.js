...
`;
    ...
    let allTypeInfos = [];
    if(type === 'todos') for(let n in allTodosObj) allTypeInfos.push({name: n, mark: allTodosObj[n].mark.toLowerCase(), tag: allTodosObj[n].tag.toLowerCase(), date: allTodosObj[n].date.toLowerCase()});
    ...
    else if(type === 'urls') for(let o of allUrlsArr) allTypeInfos.push({name: o.title, url: o.url.toLowerCase()});
    ...
    else if(type === 'texts') for(let n in allTextsSnippetsObj) allTypeInfos.push({name: n, contentLength: allTextsSnippetsObj[n].txt.length});
    ...
    if(type === 'todos') {
      ...
      arrayForGiveInfo = arrayForGiveInfo.filter(o => !allContentLngs || allContentLngs.find(l => +l === o.contentLength));
    }
    else return '';
    ...
    arrayForGiveInfo =
      ...
      : type === 'codes' ? arrayForGiveInfo.map(o => `name: ${o.name}, language: ${o.language}, content length: ${o.contentLength}`)
      : arrayForGiveInfo.map(o => `name: ${o.name}, content length: ${o.contentLength}`);

    return `[Відповідь системи на команду get: ?get| ${aiGetter}]:
━━━━━━━━ ${aiGetter} ━━━━━━━━
